...
class Settings:
    ...
    vad_mode: int
    barge_in: bool
    output_echo_guard_seconds: float
    speaker_voice: str
    ...
    def from_env(cls) -> "Settings":
        ...
        state_dir = (
            Path(os.getenv("TIPI_STATE_DIR", "data/voice")).expanduser().resolve()
        )
        ...
        token = os.getenv("TIPI_GATEWAY_TOKEN") or os.getenv(
        ...
        return cls(
            ...
            agent_timeout_seconds=float(os.getenv("TIPI_AGENT_TIMEOUT_SECONDS", "75")),
            ...
            vad_mode=max(0, min(3, int(os.getenv("TIPI_VAD_MODE", "3")))),
            barge_in=_as_bool(os.getenv("TIPI_BARGE_IN"), False),
            output_echo_guard_seconds=max(
                0.0, float(os.getenv("TIPI_OUTPUT_ECHO_GUARD_SECONDS", "0.45"))
            ),
            speaker_voice=os.getenv("TIPI_REALTIME_SPEAKER_VOICE", "cedar").strip()
            or "cedar",
    ...
    def validate(self, require_model: bool = True) -> None:
        ...
        if not 10 <= self.agent_timeout_seconds <= 300:
            raise ValueError("TIPI_AGENT_TIMEOUT_SECONDS debe estar entre 10 y 300")

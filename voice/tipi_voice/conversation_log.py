...
_SECRET_PATTERNS = (
    ...
    re.compile(
        r"(?i)\b(api[_ -]?key|token|password|contraseña)(\s*[:=]\s*)([^\s,;]+)"
    ),
)
...
class ConversationLogger:
    ...
    def event(self, event_type: str, message: str = "", **fields: Any) -> None:
        ...
        safe_message = _redact_text(message).replace("\r", " ").replace("\n", " ").strip()
        ...
        detail = " | ".join(
            f"{key}={self._format_readable(value)}" for key, value in safe_fields.items()
        )

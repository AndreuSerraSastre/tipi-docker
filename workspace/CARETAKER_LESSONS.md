...
- Observación y evidencia: el detector exigía dos hipótesis parciales, aunque un «Tipi» real puede aparecer una sola vez y mutar enseguida a «ti». Con habla solapada, el reconocedor abierto llegó a oír «tv», mientras una gramática limitada recuperó «tipi» pero también forzó falsos positivos con «sí», «tipo», «típico» y «para ti». Una reproducción de «Sí, ese tipo típico es para ti» produjo de forma intermitente `si ese tipo tv marti` y demostró que dos muestras todavía no bastaban. Además, audio ya recibido podía volver a entrar en la cola después de cancelarla.
- Corrección aplicada: una coincidencia exacta del reconocedor abierto interrumpe con el primer parcial. Bajo solapamiento se usa un segundo reconocedor sensible, pero requiere cuatro hipótesis consecutivas y una pista fonética independiente situada al final de la frase o seguida de una orden de parada; también se descarta si viene precedida por «tipo», «tipos», «típica» o «típico». El bloqueo durante playback baja a 450 ms y todo audio antiguo se suprime hasta la siguiente transcripción humana. Interrumpir ya no abandona una consulta activa, por lo que la nueva frase puede dirigirla.
- Prueba final: 58 pruebas ARM64 y una omitida, más una prueba repetida con Vosk real: «Tipi» limpio detectado a 1180 ms, mezclado con habla detectado a 2140 ms y 24 decodificaciones independientes de la frase conflictiva sin ninguna activación. También se verificaron cooldown, descarte de audio atrasado, conservación de tool call y orden estricto de eventos del gateway.
...
- Estado: superada por la validación acústica de sala descrita en la lección siguiente; cuatro hipótesis consecutivas reducían demasiado la sensibilidad real.

## 2026-07-15 — la simulación no sustituía una prueba acústica de sala

- Fecha y desencadenante: comprobación final de tres interrupciones reproduciendo voz solapada por los altavoces reales y capturándola con el micrófono USB del robot.
- Observación y evidencia: la señal sintética directa superaba el detector, pero la grabación física dio cero activaciones. El reconocedor abierto sí oyó cada «Tipi» como un alias terminal `tv` y el restringido obtuvo `tipi` en dos casos, pero las hipótesis coincidían durante un solo bloque de 20 ms; en el tercero el restringido no produjo texto después de reiniciarse.
- Qué se hizo bien: la voz se marcó como detenida intencionadamente, se aisló el ensayo sin red, se conservaron las grabaciones al fallar y un `trap` restauró el servicio sano en todos los intentos.
- Qué se hizo mal o se asumió sin pruebas: se había aceptado una mezcla digital como prueba suficiente y se exigieron cuatro hipótesis consecutivas para compensar falsos positivos. Esa regla estaba ajustada a la simulación, no al comportamiento temporal de Vosk después del trayecto altavoz-sala-micrófono.
- Impacto real o potencial: el robot podía reconocer «Tipi» en sus dos transcripciones y aun así continuar hablando, que era precisamente el fallo observado por el usuario.
